pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    ...
                    if (branch == 'user_project_management') {
                        echo "👤 Branche Raouaa"
                        env.BUILD_BACKEND = changes.contains('project_management/src') || changes.contains('project_management/pom.xml') || changes.contains('project_management/Dockerfile') ? 'true' : 'false'
        ...
        stage('Deploy K8s') {
            ...
            steps {
                sh 'kubectl apply -f project_management/k8s/'
            }
        }
    }
    post {
